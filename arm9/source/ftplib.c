...
// --------------------------
// New functions for the DS. They do essentially the same as the old functions, but will
...

GLOBALDEF int FtpDirBuf(char *buffer, int size, const char *path, netbuf *nControl)
{
    return FtpXferBuf(buffer, size, path, nControl, FTPLIB_DIR_VERBOSE, FTPLIB_ASCII);
}

GLOBALDEF int FtpAccessBuf(const char *path, int typ, int mode, netbuf *nControl,
    netbuf **nData)
{
    char buf[256];
    int dir;
